...
namespace BookStore.CODE.DuLieu
{
    public class TaiKhoan
    {
        ...
        public bool DangKyTaiKhoan(string taiKhoan, string matKhau, string ngayTao)
        {
            ...
            string sql = "insert into TAIKHOAN(Email,MatKhau,HoTen,NgayTao,MaQuyen, TinhTrang) values('" + taiKhoan + "','" +   matKhau + "',N'" + hoTen + "','" + ngayTao + "',2,0)";
        ...
        internal bool CapNhatTaiKhoan(string idtk, string sdt, string hoTen, string gioiTinh, string diaChi, string ngayCapNhat, string tenAnhDaiDien)
        {
            string sql = "update TAIKHOAN set  SDT='" + sdt +
                "', hoTen=N'" + hoTen + "', gioiTinh=N'" + gioiTinh + "', DiaChi=N'" + diaChi + "', AnhDaiDien=N'" +
                tenAnhDaiDien + "', NgayCapNhat='" + ngayCapNhat + "' where id=" + idtk;
        ...
        public int taiKhoanTonTai(string email)
        {
            string sql = "select count(ID) from taikhoan  where Email='" + email + "'";
